 ...
 * @param {string}   shiftPattern  "12_hours" or "8_hours"
 * @param {number}   maxShiftsPerWeek Maximum shifts a nurse should do per week
...
export function createBalancedScheduleEntries(
  ...
  const maxConsecutiveDays = shiftPattern === "12_hours" ? 3 : 5;
  ...
  for (const day of days) {
    for (const shift of SHIFTS) {
      for (const divId of Object.keys(acuityTargets)) {
        ...
        // ── Rule 2: night → cannot do day/morning next day ─────────────
        if (t.lastEntry && t.lastEntry.shift === "night" && (shift === "day" || shift === "morning")) {
        ...
        if (t.streak >= maxConsecutiveDays) continue;

        // ── Rule 4: Max shifts per week ──────────────────────────────
        if (t.shiftsThisWeek >= maxShiftsPerWeek) continue;
...
Rules:
1. Days: ${days.join(', ')}
2. Shifts per day: ${SHIFTS.join(', ')}
3. Nurses available: ${JSON.stringify(nurseData)}
4. Max shifts per nurse this week: ${maxShiftsPerWeek}
5. A nurse can only work ONE shift per day.
